...
| Mirrors / copies | 🔴 **DON'T HAVE THEM.** A pointer is not a copy. See §4.13 — the v1 answer here was *"sync them in the same command that verifies,"* which institutionalised the defect instead of removing it. |

**Then add a STALENESS LAYER** that fails the build if any generated document differs from what the
generator would produce, if a doc names an old chapter, **if a second copy of a tracked document
exists at all**, if the schedule misses a chapter, or if any file presents a superseded value as
current.
...
3. Update every file that records X (codex, status, plan, inventory). 🔴 Not "and the mirror" —
   there is no mirror any more (§4.13). If you find yourself writing *"update the copy too,"*
   the copy is the bug.
...
18. `sh checks/run_all.sh` — **thirteen layers, must exit 0.** It regenerates `state.json`,
    `LIN_HAO_STATUS.md`, `CONTINUATION_PROMPT.md` and the ensemble footer blocks, and **fails if any
    second copy of a tracked document exists.**
...
- **There is exactly one copy of every tracked document.** Five paths are named in
  `verify_stale.py`'s `FORBIDDEN_SECOND_COPIES`; if any of them exists, the suite fails.
