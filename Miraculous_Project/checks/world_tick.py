...
# Recomputes every entity's clock from actual PROSE (footers don't count as
# life), flags aging actors, and emits the pressure questions. Fails only if
# the registry is unparseable — pressure is information, not verdict
# (pressure PROPOSES, never mandates; an honest HOLD is an answer).
    ...
    if re.match(r'^\| W\d+ \|', line):
        ...
        if len(c) >= 6:
            rows.append(dict(id=c[0], name=c[1], kind=c[2],
                             ...
                             goal=c[4], move=c[5]))
if not rows:
    print('world_tick: FAIL — WORLD_STATE.md unparseable (need | W<n> | rows)'); sys.exit(1)
...
if press:
    ...
    W('  no flagged entities — the near-world is young; the far-world is listed in WORLD_STATE.md')
W('  RULE: pressure PROPOSES, never mandates — an honest HOLD is an answer; accidental silence is not.')
W('=' * 62)
print('world_tick: OK')
