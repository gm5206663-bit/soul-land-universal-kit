# OPEN RULINGS — FINAL LEDGER (v2.1, 2026-09-24)

## SETTLED BY THE AUTHOR (locked — full verbatim record in RULINGS_LOG.md)

| Topic | Ruling |
|---|---|
| The cheat | The System — not a martial soul; slots by category; real-time gains from the first percent; user's best speed; stage upgrades at 100%; rings age; bones age |
| The unified slot law | "slots number's always same in every list not different" — 1/1/1 at Scholar, +1 to all lists per rank-up, 6/6/6 at Spirit King, 10/10/10 at Titled Douluo |
| The rebirth | "He reborn from start, system awaken after martial soul awakening" — Earth soul in the body from the first breath; the System wakes after the awakening |
| The holder | **Su Ping (苏平)** — average innate power, Fruit Creek's only spirit child |
| The martial soul | **An apple** — food-type tool spirit on canon receipts (Oscar precedent; Food God ceiling); support-only, never strikes |
| The era | Soul Land 1, light cameos — own path, own academies |
| The opening | Page 1 = the awakening day, age six; **"He join academy of course"** — Yunxi Junior Spirit Academy soon after |
| The System's voice | Silent — a readout, not a personality |
| The Panel law | Printed panels at key beats only; prose between |
| The Incantation law | "He doesn't shout, he said in mind" — the chant is spoken in his mind |
| The First cultivar | "Coultivation speed increasing one" — the first ring's apple increases cultivation speed |
| The Mechanics law | Stored-only aging (fused-aging = rank-91 capstone); bones age like rings; aging never changes skill identity; lost techniques slottable if learned enough; god-level ruled when reached |
| The Progression law | "Complete not you thinking, it's natural what logically not fixed, it's according to World" — no fixed ladder; the world itself defines each art's next stage |
| The places | **Fruit Creek (果溪) · Yunxi Town (云溪) · Yunxi Junior Spirit Academy** |
| The village truth | "Even having soul' power is big thing for a village" |

## SYSTEM-PERFECTION ROUNDS (SC3) — ALL LOCKED

| Law | Ruling |
|---|---|
| THE AGING LAW | Never fixed — organic: the user's strength, the ring/beast's own potential, bloodline, and what the world weighs |
| THE NO-CEILING LAW | Any ring can age onward without end; potential shapes the road, never the destination |
| THE TENDING LAW | The System accepts nothing directly; the only lever is growing the man |
| THE PRICE | Free — no cost at all |
| SLOT RULES | Basics understood = slottable; free switching; progress always kept |
| THE ACQUISITION LAW | Seeing once is enough — witnessed arts become slottable; every art still starts at 0% |
| THE PASSIVE CULTIVATION LAW | A slotted cultivation method generates real spirit power 24/7 — ranks come with time itself |
| THE RESONANCE LAW (+ scope) | Slotted things influence each other — across ALL categories; the loadout is one living formation |
| STACKING | One thing, one slot — combos through resonance, never repetition |
| NO INTELLIGENCE | The System is silent by nature — law, like weather; it cannot explain itself, ever |
| PANEL STYLE | Minimal lines — 「Basic Swordsmanship — 34%」; sparse, cold, quiet |
| THE FIRST SLOT | Basic Body Conditioning — everything grows on the root |

## HELD FOR THE AUTHOR'S OWN HAND

| Topic | Status |
|---|---|
| **The System's name** | **"I give system a name"** — the author's gift, awaited. Working placeholder in all prose: "the readout." Give it whenever — even mid-drafting; it slots in without breaking anything. |
| **The completed method's name** | **"The Still Well Method" — PROPOSED in Chapter 3** (SC15 completed the event in-chapter; the name threads "I sit still well" + the well of his spirit power). The author may strike and give his own — it slots in clean. The System's name remains the only gift held open. |

## HELD FOR DRAFTING (agent proposes at the named arc; author strikes or keeps)

| Topic | When |
|---|---|
| The chant's exact words | Arc 1 (Chapter 1) |
| Page-1 details (the ceremony's cast, the day's shape) | Arc 1 |
| The first beast | **SETTLED (SC14): a Blue Silver Grass at ~99 years** — the white-class peak, plant-for-plant, life-for-food; the cultivar it shapes is the cultivation-speed apple |
| **The second beast (ring two)** | **OPEN — Chapter 4 ends on the question** ("The question is never whether. It is which."). The second seat: under 764 years by the Hall's rule of thumb; the true limit the body, the soul, the spirit; the east-of-the-river licensed stands on the page. The author rules it, as he ruled the first (SC14). |
| The first team, the first rivals | Arc 2 |
| Later cultivars (recovery, cleansing, body, flight, the golden line) | Arc by arc |
| Future System categories (S13) | Author's word only, ever |
| God-level sight; rank-100 behavior (S10) | When the story reaches it |
