...
foundation/OC_STATUS.md + the library's dd-status.html, generated by
soul_land_devouring_dragon/tools/build_oc_status.py from the authority files
(HIS_STATUS_PANEL, CONTINUITY, chapter footers, the serial README's LIVE
EDGE/NEXT BEAT). AT A GLANCE + the full sheet §1–§8 verbatim + the life ledger
(all 24 chapters, DL spans, measured words) + live edge + next chapter.
CONTINUOUS BY CONSTRUCTION: the ship script regenerates it on every ship; the
Sentinel now carries two enforcing checks (sheet stamp vs disk; site page vs
library edge) — 25 checks total. Same session housekeeping: HIS_STATUS_PANEL
brought current (territory and age lines were stale at ch21; Ch 23/24 closes
appended), four stale chapter footers corrected to measured truth (ch23 worst:
2,127 claimed vs 2,039 measured), the serial README's duplicated sentence
fixed. OC STATUS LAW written into RAILS.)
