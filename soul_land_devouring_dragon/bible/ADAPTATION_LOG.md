# ADAPTATION LOG — the Devouring Dragon serial
Law: one Status Delta per chapter; exact figures here and in STATUS_PANEL only;
beast-form roads per ADAPTATION_MODULE (senses / devouring / instinct-correction /
body). The Talent is never named; deltas record its RESULTS.

## Phase 0 — in the egg (recorded s11; happened before Chapter 1)
- The Talent was ACTIVE FROM THE FIRST MOMENT OF HIS BEING (author s11: "built
  foundation from birth mother womb"). In the dark of the egg, never sleeping,
  never waiting, it built his complete foundation: the bloodline's first true
  expression; vitality denser than a hatchling of his kind should have; the
  sensory apparatus laid down deeper; the spirit-mind substrate seeded (the
  reason his mind road can lift past the species curve); the roots of instinct
  pressed into shape. State 0→1 happened before the first breath. This is the
  mechanism of his being different and special from his own race: what hatched
  was hunger and flesh with a FOUNDATION under it. Nothing after this phase is
  granted that this phase did not earn (anti-inflation untouched).

## Status Delta — Chapter 1 close (DL 3661; hatch → edge of cold season, ~3 months)
- Situation: hatched alone (the only egg, DS-09); first season survived on the
  scree slope above the forest's first trees; den moved from the egg-hollow to
  the roots of a fallen giant at the treeline.
- What drove adaptation: the mountain itself — hunger cycles, weather endured,
  the slope's prey-web, the great wings, the lodge-smell on the night wind.
- SENSES road: hatchling baseline (blur, one-wind) → state 2 developing: the
  slope resolved into a country with distances; the wind resolved into many
  things with places and stories; the far valley gained folds. Exposure-fed,
  passive sharpening — no granted scope.
- DEVOURING road: FIRST ACTIVATION. Shell-eating (first flicker: eating =
  becoming, blood-level knowledge); beetles grazed; the first stone-runner taken.
  Conversion at sliver-rate (DS-04): growth read as warmth-along-the-body, never
  as a leap; the DEEP SLEEP after fullness established as the conversion window
  (DD-03). He woke bigger and sharper — the mountain paid back what it charged.
- INSTINCT-CORRECTION road: one mistake paid ONCE — the straight rush (jaw on
  stone), then the stillness-and-waiting that is not in his kind's playbook
  (learning road; never named). CORRECTED (s5): this road is LEARNING, not the
  Talent's portrait — the engine's portrait is the adaptation ledger below.
  The pay-once habit is now permanent.
- BODY road (ADAPTATION ENGINE, s5 doctrine): hatchling form → juvenile
  development, all of it passive and self-growing: the new scales closed harder
  than they hatched and the cold was shut out without any doing of his; limbs
  finding thickness, claws to their cold shine, tail learning its balance; the
  hollow outgrown without noticing. The engine ran from the first day (never
  dormant).
- BLOODTHIRST ledger (DS-08): met for the first time — the red pull to chase the
  winged hunter, bigger than the fear; NOT followed (the quiet thing held him to
  the rock). The flaw is real, uncured, and survived him once already.
- MEN ledger: lodge-smell received (smoke, iron-sourness, wrong-smell); body-law
  response (neck-scales standing, back turned to that wind, facing the forest).
  No human seen; nothing staged; the direction is known and kept.
- Cultivation (panel; s7 correction — SB-01, no clock): REAL AGE ≈ 3 months;
  CULTIVATION-AGE ESTIMATE ≈ 5–8 years — below the ten-year class (begins at
  cultivation-year 10). Growth ahead of ordinary beasts (species trait DD-04)
  and ahead of his kind's average (Talent, whole-existence scope); no clock,
  no leap, no number in prose.
- FULL-SCOPE NOTE (s7): the spirit-mind road and the bloodline road were
  already operating at the hatch, silently — the mind that learned the slope
  and paid the lesson once, and the blood that took every sleep and every meal
  and refined itself a little more. Recorded now; shown increasingly in prose.
- New pressures: the cold season arriving (stone-runners growing fat, slow,
  scarce); the pull toward the deep country begun; the lodge-smell on the wind
  stays where it is.

## Status Delta — Chapter 2 close (DL 3661–62; the cold season; real age ≈ 3 → ≈ 8 months)
- Situation: the whole cold season lived on the white mountain; den held (the
  roots-house); scarcity true and long; the season ends in the first waits of
  the thaw.
- What drove adaptation: snow (new medium), hunger made law, the sighting of
  men, the offal test, the first true fight, the deep winter rhythm. NOTE (s5
  correction): the whole season was one long adaptation trial — the engine ran
  all winter, feeding on cold/emptiness/digging/waiting and paying it back as
  fitness; no dead season, no dormancy.
- SENSES road: state 2 → 3 developing — scent-through-snow learned (morning snow
  tells truth, evening snow tells lies; drift keeps nothing, crust keeps
  everything); warmth-under read through crust to wintering stone-runners; the
  snow's sound-map and the spirit hounds' question heard whole. Exposure + need fed it;
  nothing granted.
- DEVOURING road: digging-out of wintering stone-runners as the scarcity economy
  (earned, small, bitter); FIRST CARRION (the offal at the pass trail — he ate
  what he did not kill; devourers keep no pride against hunger); the hill-cat
  devoured after the jaw-hold kill; conversions in the deep sleeps throughout
  the season (DS-04 sliver law: growth read as bigger/harder at each waking,
  never as a leap).
- INSTINCT-CORRECTION road: the fight's lesson — wait INSIDE the fight for the
  moment the fight gives; the jaw-law (take, and keep, and what you keep becomes
  you) as the species' deep word, held against the red thing's loud way; the
  pay-once habit kept (no repeat of the straight rush).
- BODY road (ADAPTATION ENGINE, s5 doctrine): the winter's trials met by
  self-growing adaptation — winter-down beneath the scales where the cold found
  its ways; the blood's deep roads keeping warmth where it mattered; the belly
  made efficient (nothing left in a stone-runner); digging-claws thickened blunt
  for frozen grit; after the fight, scales came hard armor-over-armor where the
  claws had found their way in; size at the storm-waking visibly beyond the
  autumn self. THE SCAR behind the jaw — for life (honest cost, kept). ALL of
  this passive, automatic, none of it done by him.
- AMBIENT DRAWING (s5 doctrine): the mountain air is not empty — the world is
  full of the faintness; his body drew on it in the deep sleeps and the still
  ones, passive as roots on water, through the whole starvation season — this is
  why the winter that ate him also built him, and why no season of his can ever
  be dead.
- BLOODTHIRST ledger (DS-08): TWO tests this season. (1) The moonlit nights —
  the lights beyond the shoulder, the red pull toward the lodges' endless food;
  held at the treeline by the quiet thing + the shape of the cold shining death.
  (2) Bloodthirst-at-the-feast over the cat-kill — pulled into the deep sleep by
  fullness before it could spend. Flaw uncured, twice survived; it grows as he
  grows.
- MEN ledger: scent → SIGHT. Spirit hounds (with questions; one asked, the wind answered);
  the upright things watched at distance killing a deer-kind from afar with the
  cold shining shapes (iron at a distance — outside every law of the kill he
  knew); the EMPTY LODGE read last — men breathe like a season, come and go with
  the cold; the mountain is the mountain under them. No contact, no staging; he
  was never known to be there.
- Cultivation (panel; s7 correction — SB-01, no clock): REAL AGE ≈ 8 months at
  the thaw-waits; CULTIVATION-AGE ESTIMATE ≈ 15–20 years → TEN-YEAR CLASS
  (white-ring tier). No arithmetic pace — growth moves with events (DS-05 s7).
  First barrier ahead: cultivation-year 100 (SB-03), unaged. Growth steady
  through the season's deep sleeps; no leaps.
- SPIRIT-MIND ROAD (s7): the mind deepened through the winter faster than his
  species' curve — the keeping of shapes (men, iron, the spirit hound-question, the
  empty lodge) is itself intellect growing: experience becoming structure.
  The canon low-intelligence of Devouring Dragons is the species baseline, not
  his ceiling (master document scope).
- BLOODLINE ROAD (s7): the dragon blood refined all season — every kill, every
  deep sleep, every trial taken into it; visible as the hardening and the
  growing-into-shape of his kind's blueprint (SB-07: pure bloodlines refine
  more than re-shape). Recorded now; it will show.
- New pressures: the thaw coming and the forest's pull grown with the winter;
  the white woods kept closed in his body (spirit hounds' questions filed there); the red
  thing and the quiet thing both grew.

## Status Delta — Chapter 3 close (DL 3662; thaw → first deepening of summer; real age ≈ 8 → ≈ 18 months)
- Situation: the slope world left behind; he is OF THE FOREST now — the ravine
  den (roots over the stream; the way out is not the way in). The green country
  gives; the winter's whole teaching has become ground under him.
- What drove adaptation: the thaw's release (the missing condition met — master
  §16.1 rapid release), the deep country itself, the first spirit-beast prey,
  the witnessing of the ring.
- CUMULATIVE LAW SHOWN (§23.3): known threat families cheaper — the slope is a
  slope; the digging is habit; the cold is a memory; the broken-pine beaks,
  once a not-yet, taken this year on scales the autumn did not have.
- SENSES road: state 4 → 5 consolidating — the world's MORE now legible: the
  plain warmth and the warmth-with-more told apart at every distance (the step
  came through the deep sleep after the spirit-beast devouring; evidence:
  changed reading of the green country; scope: the outer forest).
- DEVOURING road + RACE-SKILL ROAD (s10 correction: the race ability was being
  logged as generic devouring — WRONG; it is Devouring Heaven and Earth, DD-09,
  waking): THE FIRST SPIRIT-BEAST PREY — the bristle-back of the stream bend
  (ten-year class; carries cultivation in its flesh). Causal sufficiency
  test passed: possibility (the forest's true people), input (the winter's
  whole apprenticeship), substrate (a devouring nature made for exactly this),
  resources (patience + the bristle-wounds + risk), construction (the faintness
  taken WHOLE from living cultivation — deeper conversion than any ordinary
  meat), integration (the deepest deep sleep), evidence (waking with the more
  legible), scope (still a juvenile of the ten-year class). This is the
  qualitative step of the chapter; no fiat in it. RACE SKILL: the taking beyond
  the touching (the air drunk from a breath's distance) = first function of
  Devouring Heaven and Earth — state 4 developing, unstable, breath-range,
  cultivation-flesh only (stage record in ADAPTATION_MODULE §3c).
- SPIRIT-MIND road (s7 scope): the kept shapes have begun to hold RELATIONS,
  not only shapes — the lodge breathes with the season; the spirit hounds' question has
  a range; and now the deepest relation of all, wordless and kept: the upright
  things TAKE THE YEARS. Intelligence deepening faster than the species curve;
  still sensation + instinct + learned shapes; no human-parity thought.
- BLOODLINE road (s7 scope): the blood ANSWERED the deep forest — belonging
  before learning; the Earth-Dragon line's quiet recognition of its own
  country. Subtle; unstaged; recorded.
- BODY road: visible refinement (SB-07) — the beak-strikes turned on the
  scales; growth into the new skin; bristle-cuts healed quiet and complete;
  "a shape he was still growing into."
- NATURE-MASTERY road: bloodthirst ledger — a NEW form of the red thing rose
  over the emptied prey (deeper than hunger-rage; the first of its kind); held
  by the quiet thing + the whole winter's teaching; survived; the deep place
  made. The bloodthirst is met now as mastery grows — the flaw and its rider
  both deepen.
- MEN ledger: SIGHT → WITNESS. The ring seen: the years leave the body as a
  turning ring of pale-gold light (hundred-year class per SB-02) and are taken
  in by an upright one. He knows now, wordlessly: men do not only kill — they
  take the years. He was never known to be there. The fires still burn beyond
  the shoulder; he looks at them without the red pull and without the flatness
  of fear.
- Cultivation (panel; no clock, s7): REAL AGE ≈ 18 months at the deepening of
  summer; CULTIVATION-AGE ESTIMATE ≈ 30–40 years — TEN-YEAR CLASS, mid; the
  first spirit-beast devouring was a qualitative step inside the class. First
  barrier ahead at cultivation-year 100 (SB-03), unaged. No leaps beyond the
  causal chain above.
- New pressures: the green country's true people (the warmth-with-more) are his
  prey-web now; the shape of the year-takers kept in the deep place; the forest
  above him (things that rule valleys) unread; the lodges' next season open.

## Status Delta — Chapter 4 close (DL 3662, deep summer's crown; real age ≈ 22 months)
(reconciled session 14 — the s13 mirror script failed before this delta landed)
- Situation: the full summer lived; the prey-web is the warmth-with-more now;
  the ravine den held; the country is known and KEPT as a map (spirit-mind).
- RACE SKILL (DD-09): state 4 CONSOLIDATING — the pull reaches a STRIDE (was:
  a breath); comes at the meal's START; answers his attention (the Talent's
  refinement: waste-narrowing, binding the pull to attention — master §10.4).
  Failures kept honest: storms of hunger, blood up, small/faintly-lit prey.
  THE LOOP SHOWN: cultivation meals feed growth, growth feeds the skill, the
  skill feeds the taking — neither began it, neither does it alone.
- SPIRIT-MIND road: the kept shapes now hold RELATIONS enough to MAP the
  country whole — beneath, the law of the jaw; above, the rulers of the
  valleys; outside the law, the takers of the years. Still wordless keeping.
- NATURE-MASTERY road: THE RIDER FINDS THE REIN — bloodthirst pointed at the
  moonlit break; the quiet thing held up the ledger of his life; he WAITED by
  history, not fear; the straggler taken clean (direction, not suppression —
  DS-08: the flaw remains, uncured).
- BLOODLINE road: suppression deepens naturally with growth (the undergrowth's
  long slow wave before him); SUPPRESSION'S REVERSE FELT — the black-ravine
  Black-Ravine Serpent's weight from above (the law one-directional down); KINSHIP MARKED —
  a thread of the dragon in the Black-Ravine Serpent's blood (Bloodline-Horizon law:
  marked, never pursued).
- BODY road: SB-07 visible — breadth of chest, darkness of scale laid layer
  upon layer (the species' armor coming), reach of jaw.
- DEVOURING road: spirit-beast hunting as the summer's bread (the fern-deer
  [design]); plain warmth as honest meals; the husk of the year-takers left
  uneaten (wrongness, not law).
- MEN ledger: proof renewed — the wounded prey he tracked, found already
  husked; the kept shape writes: they hunt what he hunts, the sharing runs
  against him; spirit-spirit hound voice far across the valley, gone by evening; no contact.
- Cultivation (panel; no clock, s7): REAL AGE ≈ 22 months; CULTIVATION-AGE
  ESTIMATE ≈ 50–60 years — TEN-YEAR CLASS, mid toward late. No leaps.

## Status Delta — Chapter 5 close (DL 3662 → second cold season's setting-in; real age ≈ 30 months)
- Situation: the second autumn met (not endured); the ravine den held; the map
  of the country kept true (the pass trail wide-grounded, the black ravine
  wide-grounded, the lodges looked at with the whole of his years).
- BLOODLINE road: suppression grown with growth — the forest lowers its voice
  farther and more wholly; the predator's paradox born (his greatness warns his
  dinner) → forced the adaptation below.
- NATURE-MASTERY road: THE VEILING — first function at the mist-morning fern-
  deer stalk; state 3→4 developing: whole attention only; holds over the
  smallest lives; breaks when the blood is up or the red thing rises; learned
  by the ledger of the eaten and the lost. By the cold season's set-in: he can
  walk his trails hushed to a shadow — THE FIRST PIECE OF THE MASTERY OF
  HIMSELF. Bloodthirst ledger: the old stag-kind fight — the red thing rose
  and the quiet thing held the jaw to its work instead (riding holds under
  strain for the first time; DS-08 honored, flaw uncured).
- DEVOURING road + RACE SKILL (DD-09): THE OLD STAG-KIND [design] — the
  forest's honest culling (ancient, sick, alone); the hardest fight of his
  life paid for honestly (horns opened his shoulder to the bone; hold slipped;
  jaw-work held); the deepest devouring: heavy slow full YEARS — the pull
  strained at them and kept taking (the skill met prey above its comfort for
  the first time and stretched); the longest deep sleep under the first snow.
  Causal sufficiency test passed: possibility (the culling of the old is the
  forest's own law), input (the wall's hunger + the veiling + a whole day of
  approach), substrate (the skill and the jaw), resources (the wound, the
  day, the risk), construction (years of weight taken), integration (the
  longest sleep), evidence (woke grown toward the wall), scope (not across it).
- THE WALL (SB-03, bottleneck law, master §24): the first barrier at
  cultivation-year 100 APPROACHED and FELT, unaged and uncrossed — growth
  slowed, deep sleeps pay fullness more than growth, the world's faintness
  thinner because HE has grown past his country's giving; the hunger that has
  no meal (MORE, GREATER, without a where). The wall is passed by the whole of
  the beast becoming greater — not by one meal however great. No arithmetic,
  no derived barrier-age; the crossing will be event-shaped when it comes.
- SPIRIT-MIND road: the map kept TRUE and extended — iron can wait: the snare
  read as the cold shining death left waiting (they can take the years without
  being there); relations deepening; still wordless keeping.
- BODY road: SB-07 visible — larger in his scales, heavier in his bones; the
  shoulder wound healing slow through the cold (a kept wound, honest).
- MEN ledger: the ring-hunters' snare [design] at the deep pass — untres-
  passed, wide-grounded; the lodges' cold-season breath; no contact, nothing
  staged. BLACK-RAVINE SERPENT ledger: the weight sleeps in the black ravine country;
  marked quietly from a distance; not finished.
- Cultivation (panel; no clock, s7): REAL AGE ≈ 30 months at the second cold
  season's setting-in; CULTIVATION-AGE ESTIMATE ≈ 85–95 years — TEN-YEAR
  CLASS, late; the wall at 100 ahead, unaged. The old stag-kind's heavy years
  moved the estimate; nothing leapt; nothing derived.
- New pressures: the wall; the white; the hunger that has no meal; the rein
  under strain; the country grown small for what he is becoming.

## Status Delta — Chapter 6 close (DL 3662–63, the second winter after the crossing; real age ≈ 34 months)
- Situation: THE FIRST BARRIER CROSSED. The white endured from mastery; the
  long storm + fasting extremity = the event; the crossing a season inside the
  storm. He is of the HUNDRED-YEAR CLASS now, at its dawn.
- THE CROSSING (SB-03; written against master §29, the Breakthrough Law):
  trigger = extremity + the skill's new reach (the pull went DOWN for the
  first time, past flesh and air, into the mountain's own old slow faintness —
  necessity, not triumph). §29.2 honored: the Talent stood over the crossing,
  reinforcing layers too weak to survive the becoming, slowing it until they
  bore it. §29.4 honored — observable consequences shown: movement changed
  (the still economy of the snow); perception expanded (the Black-Ravine Serpent's weight
  read as never before); recovery faster; the pull unchained (farther,
  easier); NEW LIMITATIONS specific to the new state — the greater hunger (the
  wall passed becomes the floor of the next hunger) and the heavier presence
  (the veiling takes strain again — part of the price, taken honestly).
- RACE SKILL (DD-09): state 4 → 5 — THE NEW REACH: the taking draws on the
  deep ambient faintness of the ground itself (the mountain's slow more),
  first under storm-fast extremity; control and generalization ahead (master
  §29.3: later use still required — fine control, safe maximum, generalizing).
- SPIRIT-MIND road: the crossing recalibrated the mind with the whole (§29.1)
  — the map steadier, the reading of greatness richer; the new knowing kept:
  the years he has crossed are his; no upright thing takes them except with
  his whole life. Still wordless.
- NATURE-MASTERY road: bloodthirst ridden AGAIN at the confirmation hunt (the
  rein holds under a true fight now); the veiling under new strain from the
  heavier presence — the work of teaching the weight to be quiet begun again.
- BLOODLINE road: the crossing refined the blood with the whole (§29.1) —
  suppression visibly deeper (the under-snow lives go down before him; the
  white country reads him changed); the dragon-thread in the Black-Ravine Serpent read
  clearer.
- BODY road: SB-07 visible at the tier crossing — larger, layered scales
  hard-broken against the long-fang's bite, healing faster.
- DEVOURING road: the greater hunger needs the greater meals — the lesser
  meals no longer answer; the outcast long-fang [design] confirmation kill
  (once wide-grounded; met in the new movement; taken clean).
- MEN ledger: the snare under the snow — iron can wait; the lodges silent in
  the dead of the white; no contact, nothing staged.
- Cultivation (panel; no clock, s7): REAL AGE ≈ 34 months; CULTIVATION-AGE
  ESTIMATE ≈ 100–110 years — HUNDRED-YEAR CLASS, dawn (yellow-ring tier,
  100–999 cultivation-yrs); the estimate reached the wall and the wall opened
  by becoming. Next barrier at cultivation-year 1,000 (SB-03), unaged. No
  leaps — the crossing was earned by the whole chain above.
- AWAKENINGS SEIZED AT BARRIER ONE (author s17 stage-breakthrough law — every
  stage breakthrough carries the chance of awakening/evolution, and a crossing
  without its opportunity is a nerf; opportunity = chance, seized by
  foundation + extremity + the Talent, never granted): (a) THE BLOODLINE STEP
  — armor set into the full deep layering of his kind; the FIRST CROWN of the
  dragon along skull and neck (the blood's first word of what it could
  become). (b) THE SENSE OF YEARS — the world's more carries AGE (young thin
  and bright; old heavy and slow; ancient burning low and deep); the sense the
  devouring nature had been reaching for; the greater hunger feeds by it.
  (c) THE DRAGON'S VOICE (DD-11; author s21 — "logically he should have a
  skill after 100 years"): the bloodline's first ACTIVE skill — the roar
  carrying the weight of his blood; hammers the low lives flat; makes equals
  falter; barely reaches above him; short; paid in breath and blood; not yet
  at his calling (state 2–3 developing; control = the climbing); proven at
  the long-fang hunt (the falter in the air of its spring; the instant was
  his). (d) THE RACE SKILL'S DEEPER FACE — the mountain-drawing reach.
  Written into Chapter 6 prose same turn (author s21 discipline).
- New pressures: the greater hunger and the greater meals it needs (the outer
  country may not hold them forever); the veiling re-learning under a heavier
  presence; the walls ahead (1,000; 10,000; the gate); the Black-Ravine Serpent read
  clearer — and the reading is a pull of its own, kept, not followed.

## Status Delta — Chapter 7 close (DL 3663, the height of the second spring; real age ≈ 40 months)
- Situation: the deep winter after the crossing hunted by the sense of years;
  the thaw and the second spring met out of the ravine den; the outer country
  thinning under him; the pull of the deep returned — changed (belonging →
  need); the shape held without deciding.
- SENSES road: THE SENSE OF YEARS at full hunt — the world wears its years;
  he passes the bright thin lives and reads for the heavy ones; the plain
  warmth is bread, the heavy years are meat; the spring's plenty has ORDER in
  it now — the order his. The sense has no walls: it reads the Black-Ravine Serpent's
  years across the ridge as a DEPTH past all other depths (no number in him;
  the ancient ones of his country burn beside it the way the last of a fire
  burns beside the sun).
- NATURE-MASTERY road: THE PRICE OF THE BECOMING paid honest — the heavier
  presence broke the veiling where it had not broken (meals lost; the ledger
  keeps the account); the veiling re-learned — slower because the weight is
  greater, faster because the teacher is greater; holding again over the
  lesser lives by the graying of the snow. Bloodthirst ledger: paid in its
  season, ridden, uncured (DS-08).
- DEVOURING road + RACE SKILL (DD-09): THE NEW REACH GENERALIZING (master
  §29.3 later use, honestly uneven): the mountain-drawing becomes a habit of
  the still hours — thinner than in the storm's extremity (the emptiness
  reached, the fullness reaches less), deeper in sleep, deepest in the hollow
  of the fallen giant where the ravine stone is old past the forest. Control
  and safe maximum still ahead (§29.3).
- SPIRIT-MIND road: INTELLIGENCE LAW honored — the Black-Ravine Serpent's shape neither
  kept nor refused but HELD until he grows to the size of it; the map grows
  a line without a word: the sharing of the country is a race (wordless
  relation, no thought); the new truth held: the outer country cannot feed
  what he is becoming.
- BLOODLINE road: suppression visibly deeper after the crossing — the low
  lives bow deeper and farther; the first crown of the dragon READ IN THE
  BLOOD by the low things (they read the crown the way they read the weight,
  from before learning); the dragon-thread in the Black-Ravine Serpent clearer than ever;
  his blood answers with the fear and the thing that is not fear.
- MEN ledger: THE YEAR-TAKERS DEEPER — a new snare set DEEPER than winter's
  (in the country of the heavier lives); spirit hounds and flat hard voices moving
  deeper into the country of the heavy years at the last of the spring; "the
  hunger ran both ways of the trail, and the trail was one"; no contact,
  nothing staged.
- Cultivation (panel; no clock, s7): REAL AGE ≈ 40 months; CULTIVATION-AGE
  ESTIMATE ≈ 115–130 years — HUNDRED-YEAR CLASS (the heavy meals of the
  winter and spring; the deep-drawing habit between them). Next barrier at
  cultivation-year 1,000 (SB-03), unaged, unfelt. No leaps.
- New pressures: the outer country thinning (the deep country has the years
  he needs); the Black-Ravine Serpent's ground between him and the depths; the year-takers
  racing for the same years; the pull of need pointing where the pull of
  belonging pointed; the shape not ready for the deciding.

## Status Delta — Chapter 8 close (DL 3663, the first hunting of the deep country; real age ≈ 44 months)
- Situation: THE DECIDING done as non-decision (the shape ripened until the
  beast was doing what the shape had always said); the outer country kept
  whole in the keeping, past argument; the Black-Ravine Serpent's ground CROSSED; a new
  hollow made in the deep country (the way out is not the way in — the first
  winter's law kept in all countries). The keeping of the deep country is NOT
  yet earned; the earning has begun.
- BLOODLINE road: THE BLACK-RAVINE SERPENT'S GROUND CROSSED by patience, the rim-path, and
  the knowing of when the weight sleeps (the ancient ones have their
  stillness); the weight moved mid-crossing — he became nothing (veiling to
  its last shadow, body to stone, breath to nothing); the stirring passed;
  the dragon-thread sang its one thin note and was quiet; teaching kept: the
  greatness above is not malice and not mercy — its ground is paid for in the
  whole of the care. THE BLOODLINE DIVIDEND MADE EXPLICIT (DS-10 the spine,
  shown in story): the deep-fringe devouring sorted without being asked — the
  lesser burned away into body/strength/instinct; the deep and true of it
  went down into the dragon's blood and the blood was MORE DRAGON for the
  drinking; the purification found its true country; scales darker and
  deeper; the crown a shade higher. NO PREY-MUTATION; the fuel law honored.
- SENSES road: the sense of years reeled at the deep country — depths
  everywhere, and depths beyond its reading (greatnesses standing as the
  ABSENCE of reading; the sense bows; the map has room again); its true gift
  shown at the crossing: the reading of the greatness above him.
- NATURE-MASTERY road: BLOODTHIRST THE STRONGEST IT HAS EVER BEEN — the
  years' blood in him like a fire; the red thing said THERE and AGAIN; the
  quiet thing stood with the whole of his life in its keeping; the rein held,
  and the holding was harder than it had ever been (DS-08: the flaw climbs as
  he climbs). The CROWN read by beasts that never bowed to his kind: the
  lesser deep lives bow; the not-bowing came in kinds — blood that remembers
  the dragon's word and gives ground; and weights that answer his weight (the
  wide ground given — some of the deep country's words are older and heavier).
- DEVOURING road + RACE SKILL (DD-09): the first hunting of the deep fringe —
  chosen with the whole of the care (not the greatest, not the least; alone;
  old near the end; unaware); the hardest fight of his life (hide the jaw
  must find its way through; strength that stood up out of the first spring;
  the patience of the years); taken by the whole of what he was — veiling,
  the spring, THE VOICE buying the falter (DD-11 proven again), the jaw's
  hold; paid in the torn shoulder and the opened flank. THE KILL IS A WORD:
  the years' blood smells go where smells go; lives heavier than his followed
  the kill; he ate fast and was gone before the answering came — in the deep
  country the beast that speaks the word must be gone before the answering.
  The pull drank deep: the years of the deep beasts are not the years of the
  outer country.
- SPIRIT-MIND road: INTELLIGENCE LAW honored — wordless keeping; the map has
  room again (the room is the whole of the deep country); the race-line kept:
  the year-takers in the deep country for the same years; the sharing is
  still the race, and the race has no end in the reading.
- MEN ledger: spirit hounds and flat hard voices down-country of the giants — the
  year-takers have come into the deep country; no contact, nothing staged.
- Cultivation (panel; no clock, s7): REAL AGE ≈ 44 months; CULTIVATION-AGE
  ESTIMATE ≈ 130–145 years — HUNDRED-YEAR CLASS (the deep-fringe devouring
  moved it; the bloodline dividend is quality, not a leap). Next barrier at
  cultivation-year 1,000 (SB-03), unaged, unfelt. No leaps.
- New pressures: the keeping of the deep country to be earned; the kills that
  are heard; the year-takers in the same country; the greatnesses past his
  reading; the bloodthirst climbing with the blood; the Black-Ravine Serpent's ground now
  behind him, between him and the country that made him.

## Status Delta — Chapter 9 close (DL 3663, the deep summer's height; real age ≈ 48 months)
- Situation: THE KEEPING OF THE DEEP COUNTRY EARNED — not wholly (the earning
  has no end he can read), but truly. Trails his, hours his; the word of the
  kill his to speak and to place; the voice at his calling; the veiling held
  in the moving; the map kept true; the deep country beginning to keep him.
- NATURE-MASTERY road: (a) THE WORD OF THE KILL PLACED — the answering
  learned by kinds (scavengers of the years; hunters of the scavengers; the
  heavier lives drawn by years' blood); he eats the deep part and the true
  and LEAVES THE REST AS A WORD of its own — the rest calls the answering to
  where he is not; the first day the kill-word was his to speak, not only his
  to fear. (b) THE RED THING RIDDEN IN THE RUNNING — the standing became more
  than standing: at the last hunting of the summer the quiet thing did NOT
  hold the red thing down — the strength given its head and kept to the line
  (the line = the jaw's work); the red thing paid out of the payment; kept:
  held, and then RIDDEN — not mastery, not the flaw's ending, the beginning
  of the way the flaw will be carried (DS-08 honored, uncured).
- RACE SKILL + voice: THE VOICE at his CALLING now (first low, short, spent
  in breath and blood, but his; the calling growing truer; asked the falter,
  the falter given — state 3→4). THE VEILING'S harder climb: eyes that read
  the blood; held longer, finer, WHILE MOVING; broken, written, relearned; by
  deep season held in the moving as once only in the still.
- BLOODLINE road: THE GREATENESS PAST HIS READING READS HIM — the attention
  of the depth turned upon him (not malice, not mercy; nothing in it wanted
  him; it turned FOR him); he became nothing; AND THE BLOOD ANSWERED — the
  dragon's blood cleaned and deepened and drunk full of the deep years went
  out carrying its word; the attention held the space of a stillness; the
  nearest thing the wordless has to a word came down: the word of the dragon,
  READ AND KNOWN; the attention withdrew; HE WAS LET GO. DS-10 shown at its
  far edge: the acknowledgment EARNED by the purification — nothing granted,
  nothing staged, no contact, no lore spoken. The shape kept deeper than all
  the other shapes (it changes them: the deep country knows the word of the
  dragon, and has read the word in him). THE BLOODLINE DIVIDEND COMPOUNDING:
  the blood drunk full all summer, kill by kill and sleep by sleep — scales
  darker and deeper than any season has left them; the first crown standing
  higher; the thread of the dragon singing clearer to the thread beyond the
  ridge; the blood more dragon than at the spring; the direction has no end
  in the reading.
- SPIRIT-MIND road: INTELLIGENCE LAW honored — wordless keeping throughout;
  the lists (the answering, the hours, the waters); the map kept true under
  the closer sharing; the shape of the letting-go held whole.
- MEN ledger: THE YEAR-TAKERS DEEPER — a new husk of a beast of heavy years;
  the ring's light DEEPER than the first witnessing (darker than pale gold —
  the purple-ring tier, panel-only; he reads the color, never the name); the
  hunt itself read passing beneath his high ground in the country of the
  giants — they have learned to read the deep country too (or brought the
  reading); the sharing closer; the map kept true because the keeping is the
  whole of the care. No contact, nothing staged.
- DEVOURING road: the deep summer's heavy meals; the pull at the deep wells
  of the giants' ground; the eating sorted per DS-10 (the deep and true fed
  the dragon's blood; the lesser burned to body/strength/instinct).
- Cultivation (panel; no clock, s7): REAL AGE ≈ 48 months; CULTIVATION-AGE
  ESTIMATE ≈ 150–165 years — HUNDRED-YEAR CLASS (the deep summer's heavy
  meals; the dividend is quality first). Next barrier at cultivation-year
  1,000 (SB-03), unaged, unfelt. No leaps.
- New pressures: the earning has no end; the year-takers reading the deep
  country; the greatness that read him — the first depth that reads BACK;
  the voice and the veiling still climbing; the bloodthirst carried now as
  the ridden thing, and the riding is a road, not a finish.

## Status Delta — Chapter 10 close (DL 3663 cold season → turn of the year; real age ≈ 54 months)
- Situation: THE COLD COUNTRY — the deep country's first winter, kept him not
  wholly but truly; the deep year turned with the war of the lists opened.
- NATURE-MASTERY road: (a) THE OUTER WINTER'S LEDGER BROKE — the deep cold
  comes UP out of the ground before the sky's warning; the heavy kinds moved
  early; the first ambush stood empty for days; the lean days paid for the
  believing; every country keeps its own weather. (b) THE MAP'S LAST LAYER —
  the standings of the greatnesses held as FIXED things (the sense of years,
  cold-sharpened, reaching further than ever); the walking of the cold
  country became the walking between them. (c) THE RED THING OF THE WANT —
  scarcity's bloodthirst: the red WANT (waits, whispers, spends the hunt
  before it begins); NEAR-LOSS: the grey stag of the second water, marked in
  the ledger — the want ate the veiling on the way in, the quarry gone, the
  day lost; the lesson kept: the want sharpens the hunt or spends it, and the
  choosing is the whole of the riding; proven at the next taking (the want
  carried THROUGH the hunt as the edge, not the rider).
- RACE SKILL + voice: THE VOICE LEARNS THE COLD — spends HEAT as well as
  breath; the winter's voice is a winter's price: spent only at need, for the
  whole of the need (the miserliness climb). THE VEILING — broke under
  HUNGER where it held under fear (hunger is a louder word in the blood);
  the line kept: the veiling is fed on the ATTENTION, not the fullness; by
  the cold month's end held in the hunger — broken now by everything that
  breaks it, rebuilt after everything.
- BLOODLINE road: WHAT THE LETTING-GO RIPENS INTO (nothing staged) — the
  ripening is ORIENTATION: the greatnesses fixed in the map; the dragon-
  thread beyond the ridge carried CLEARER in the cold air; and the smallest
  deepest dividend — THE COLD HAS LESS HOLD ON HIM: the dragon's blood keeps
  its own fire (quality of the deep meals and sleeps; the fire stands in him
  in the cold the way the first crown stands on him). Visible: scales darker
  than the winter; crown higher than the summer left it; thread singing
  toward the far ridge.
- SPIRIT-MIND road: INTELLIGENCE LAW honored — the winter written down as a
  new country (pages in the ledger); the smoke-line mapped; the marked
  quarry kept in the deep place with its days and hours; the war of the
  lists written in the deepest place.
- MEN ledger: THE YEAR-TAKERS WINTER IN THE DEEP COUNTRY — the light that
  eats wood smelled from the far ridge; a winter place in the breaks; smoke
  standing day after day; the map gains the line it never held (they STAY);
  the race sharpened (same breaks, same hours, same heavy kinds; the spirit hounds'
  voices carried further in the cold air). THE TURN: they took the grey stag
  of the second water — the beast he had marked for the taking; the light
  darker than pale gold DRAWN and gone before he walked the ground (the two
  kneeling-marks pressed in the moss, the smell of the drawing still on it;
  purple tier, panel-only; color read, never named — ER-13). THE WAR OF THE LISTS: his list kept in his blood, their list kept
  in their hands, written over the same beasts; no end in the reading. No
  contact, nothing staged.
- DEVOURING road: the lean days; the warm-breaks' heavy meals; the taking
  after the want-lesson (the want as edge, not rider); sorted per DS-10.
- Cultivation (panel; no clock, s7): REAL AGE ≈ 54 months; CULTIVATION-AGE
  ESTIMATE ≈ 180–195 years — HUNDRED-YEAR CLASS (the cold country's lean
  meals chosen deep; quality first). Next barrier at cultivation-year 1,000
  (SB-03), unaged, unfelt. No leaps.
- New pressures: the war of the lists (their hands take what his patience
  marked); the cold country must keep being learned (its weather is its own);
  the voice's heat-price in the cold; the red want of every lean season; the
  standings of the greatnesses walked between; the thread's road unended.

## Status Delta — Chapter 11 close (DL 3664 thaw → green season; real age ≈ 58–60 months)
- SENSES / MAP road: THE DEEP THAW — the green came up out of the ground
  before the sky's word; the outer green's ledger failed the deep, as the
  outer cold's did (every country keeps its own weather — the winter's
  lesson carried into the spring); the deep green filed as a season already
  started taking. THE SMOKE-LINE GONE — the winter place struck at the
  thaw; the map rewrote the line it never held: they stayed, and they went,
  and the going is the going of readers (a hunting line that goes into the
  green of a country it has read goes to work, and work does not go home);
  the next smoke kept as an edge.
- SPIRIT-MIND road: INTELLIGENCE LAW honored — the green season written down
  as pages in the ledger; the marked quarry kept in the deep place with its
  days and hours and the faith of the kept-for; the war of the lists rewritten
  with its second kind of page (the line that could be lost; the taking still
  taken); the war's ledger kept: what is taken in other hands, what is taken
  in his.
- MEN ledger: the year-takers STRUCK the winter place at the thaw and went
  deep with the green (the going is the going of readers); the reader's
  parallel panel (the 2026-09-18 ruling, first use) gave them faces — OLD QIN
  (the list-keeper; SOUL KING; the stag's purple light carried down the road to
  the tables — depth for frames that have grown into it, the ring-slot law ER-12
  [rewritten s33; the s30 text read "as his SECOND RING" — struck by the author];
  "It leaves signs. Then it is a line on the list."), GAO TIE (the spirit hound's master; GRAND SOUL MASTER; "the old hunters of the border say the deep country
  has its own keepers"), XIAO AN (the young one; SOUL MASTER — a white first ring carried since the border, her second light to come [s33]; the deep kills left whole:
  "it knows the country, and it knows where a beast will stand" — the kill word read from their side); their list on paper, their motive on the
  page (the deep keeps its own green; the list is deep). No contact; nothing
  reaches him; the dragon never hears; the firewall stands.
- DEVOURING road: the lean end of the cold, then the green season's first
  deep meal — THE THICK-PAW OF THE GREEN RUN [design] taken at the window's
  middle (the race won); the taking after the want-lesson and after the
  faith-spent (the want as edge, not rider; the patience of the choosing);
  the bloodline dividend sorted per DS-10 (the lesser burned; the deep and
  true fed the dragon's blood — quality first).
- WHAT WENT WRONG (PROSE LAW s25, the failure first): THE SECOND LINE TAKEN —
  the broad-antler of the high break [design] marked with the patience of the
  marked days, taken by the year-takers at the marked hour; the ring-light
  pale gold (hundred-year class; color read, never named) — the first line
  darker than pale gold, the second pale gold, and the list counts the line,
  not the weight; the cost paid in the days of the marking, the meal not
  eaten, the faith in the ground spent twice. THE VEILING'S NEAR-MISS in the
  wide green — the crossing's heavier presence thinned the hush on the open
  run; two low lives turned their heads; the stillness cost time; the time
  cost the thick-paw's last stride; the jaw-work held the rest — the green
  is the veiling's own climb (its next truth).
- THE VOICE road: UNSPENT through the green's first taking (the taking came
  on ground and hour and jaw); the winter's price (the cold's miserliness)
  bought a summer's silence; the silence, kept, worth more than the calling
  spent — the voice's next truth.
- BLOODTHIRST ledger (DS-08, uncured): THE TWO REDS MET — the deep meal gave
  the red thing of the FULLNESS its summer back (the boil over the top of
  the meat) and put meat in the red thing of the WANT (the whisper with
  sharpness in it); the two reds stood at the same hour; the choosing stood
  against both and found the same standing (the choosing is against the
  choosing's own being taken — and is not taken); the two words of the red
  thing named at last as one thing, and the line the choosing stood on
  written in the deepest place.
- Craft: the kill word placed at the meal's end (the deep country's craft;
  his to speak, not only his to fear).
- Cultivation (panel; no clock, s7): REAL AGE ≈ 58–60 months; CULTIVATION-AGE
  ESTIMATE ≈ 180–195 years — HUNDRED-YEAR CLASS (the cold country's lean
  meals chosen deep, then the green season's first deep meal; quality first).
  Next barrier at cultivation-year 1,000 (SB-03), unaged, unfelt. No leaps.
- New pressures: the war of the lists in the green (the two lists converge
  on the same heavy kind — the season's question: the next line, and which
  of the two lists would take it); the green country must keep being learned
  (its weather is its own; its wide ground is where the heavier presence
  lives); the veiling's green climb (the heavier presence, its own
  discipline); the voice's kept silence (spent or kept, at what price); the
  standings of the greatnesses walked between in the green; the thread's
  road unended (the song does not season; nothing staged).

## Status Delta — Chapter 12 close (DL 3664 green season at its height; real age ≈ 60–62 months)
- SENSES / MAP road: THE GREEN AT ITS HEIGHT — the deep country wears the
  green wholly; the heavy kind fat in the warm breaks; THE MAP'S EDGE
  ANSWERS — the new smoke stands in the country of the giants (the place
  kept as a question with an edge, closed); the line of their going read
  down to the hour.
- THE MARK OF THE BULL: the OLD BULL OF THE WARM BREAKS [design] — years
  standing LOW and deep (purple light, panel-only; the sense of years bows
  at the edge of the reading); the deepest meal the green had to give;
  marked for a SEASON, not an hour. THE CRAFT'S COST — the warm breaks had
  shifted (the answering of the deep country: the kill word standing in the
  ground the way a name stands in a country; the heavy kind moving at the
  edges of the hours, standing away from the marked ground) — his own craft
  made the ground wary, and the wary ground moved the bull's hours: the
  word that guards the country guards the taking too (the taking came at
  the edge of the hour, on the thinnest ground, or would have been lost).
- MEN ledger: THE FIRES IN THE GIANTS' COUNTRY (their green-season camp; the hound working the water line; every country keeps its own weather, from their side); their list has grown the KEEPERS' LINE (the mark written at the winter place — the beast that leaves signs and never shows itself); their craft: the water's edge, at the water's hour, the
  retreat rule ("if the beast comes, we pull back") — their discipline
  against his word. THE CROSSING AT THE WATER: the going-out at the hour
  the meal stood begun; the veiling at its last margin; the hush held; the
  HOUND STOPPED, SAT, HELD THE SCENT; the upright shapes looked and
  found no one; the going done before the looking finished (the ground's
  word carrying him out of the ground the way the ground carries a
  secret); NOTHING KNOWN, NOTHING SEEN OF HIM — but the spirit hound CARRIED THE
  SCENT; the reader's side (the parallel panel): the spirit hound's certainty for a
  whole hour, the mark at the water, the whole body gone, the SIGN OF THE
  SEEN set on the keepers' line ("When a line has been seen, the list goes
  to it." — their next going is toward the mark). No contact; the firewall
  stands.
- DEVOURING road: THE BULL AT THE WATER — the fight the whole of what he
  was paid for (the horns against the jaw; the ground wet; the going-out's
  smell in the air); the jaw's work held against the red thing's rise; the
  deepest meal of his life; the bloodline dividend sorted per DS-10 (the
  lesser burned; the deep and true fed the dragon's blood — visible: scales
  darker, the crown higher, the dragon-thread clearer; quality first).
- WHAT WENT WRONG (PROSE LAW s25, the failure first): THE CRAFT'S COST
  (his own word made the bull wary — the taking came at the edge of the
  hour, on the thinnest ground of his life, at the water's edge with the
  going-out's smell in the air, or would have been lost — the word that
  guards the country guards the taking too); THE VEILING'S LAST MARGIN
  (the hush held through the crossing, and the after-cost came: the
  ground's word spent to the last of its margin — the deep stillness, a
  day, the country keeping its own secret about him longer than it would
  otherwise — the ground re-learning him); THE CROSSING'S COST (the spirit hound's
  certainty — the war's shape changed: the next line of theirs will come
  with EYES, not only hands).
- THE VOICE road: UNSPENT into the green (the second summer of the
  silence) and UNSPENT THROUGH A TRUE FIGHT (the bull at the water) — the
  FIRST PROOF OF THE SILENCE'S WORTH; the line of its next truth stands at
  the edge of the deep place, not yet written, not gone to (the shapes not
  yet written do not go to).
- THE VEILING road: THE GREEN CLIMB — the hush now carried by the ground's
  word, the deep stillness (the stillness learned as its own thing in the
  green); the climb paid at the water (no head turned; the hush held at
  its last margin); the hush stands in the ledger as a SPENT line, not a
  standing one — bought each hour, the margin paid at the water, the
  after-cost the deep stillness (its next truth: the green's wide ground,
  its own discipline, still ahead).
- BLOODTHIRST ledger (DS-08, uncured): THE TWO REDS IN THE DEEPEST
  FULLNESS — the boil at its height over the top of the deep meat; the
  WANT QUIET for the first time (the fullness so deep the want slept in
  it); the choosing stood against both in the quiet, and found the quiet
  was the standing, and the standing was the quiet; the red thing ridden
  in the deepest fullness as in the leanest want; the flaw uncured, and
  the flaw ridden.
- Craft: the kill word placed — the word of the bull, the deepest word yet,
  left as a name in the deep country; the country now carries his word
  everywhere the heavy kind pass — THE WAR IN THE GROUND ITSELF (his craft
  is his cover and his tell).
- Cultivation (panel; no clock, s7): REAL AGE ≈ 60–62 months;
  CULTIVATION-AGE ESTIMATE ≈ 180–195 years — HUNDRED-YEAR CLASS (the green
  season's deep meals; quality first). Next barrier at cultivation-year
  1,000 (SB-03), unaged, unfelt. No leaps.
- New pressures: the war's shape changed (their next going is toward the
  mark — the first time the war is aimed at him as a line, not at beasts);
  the word in the ground (his cover and his tell — the war in the ground
  itself); the veiling's after-cost working off (the ground re-learning
  him); the voice's next truth at the edge of the deep place; the want's
  first deep sleep against the green's deepening (does the fullness keep
  sleeping it, or does the want wake changed?); the standings of the
  greatnesses walked between in the height of the green; the thread's road
  unended (unaged; nothing staged).

## Status Delta — Chapter 13 close (DL 3664, the falling of the green; real age ≈ 64–66 months)
- Situation: THE KEEPERS' LINE — the war aimed at him as a LINE for the first
  time; the green falls; both lists on one country.
- NATURE-MASTERY road: (a) THE AFTER-COST OF THE CROSSING — the water bank
  learned him (his word over four days of bank; the small lives gone from it;
  the stillness ground remembered, as the ground remembers everything spent on
  it); the debt paid down through the season until his own bank was his again.
  (b) THE WAR IN THE GROUND — his kill-word read for the first time as a SIGN;
  his own craft turned against readers (the loud kill left in the far south,
  and again, and once in the low outer country); the feel of being a country's
  secret instead of its sign. (c) THE VOICE'S NEXT TRUTH — spent once at the
  edge of the deep place; the deep country read it as a WORD (no going flat,
  no giving of ground: the silence of being read); a voice that names him to
  anything deep enough to read. (d) THE WANT WAKENED PATIENT — held down by
  the bull's fullness through the green's height; at the turning it woke
  changed: unhurried, wanting the deep years, the country's own patience got
  into the flaw. The riding continues; DS-08 uncured.
- MEN ledger: THE KEEPERS' LINE SET — the list goes to the line it has seen;
  the watch craft (a kill of theirs left standing, the light boxed, the wire on
  both trails, the blind, the staked hound, four days to a fold, the kill
  carried north a day's walk at a time); the rule to look and not shoot and to
  write what comes; Old Qin names the line; two lodge men up from the fold;
  their paper now holds his ground and they have never once seen him.
- Cultivation (panel; no clock, s7): REAL AGE ≈ 64–66 months; CULTIVATION-AGE
  estimate unchanged in kind (past the first barrier; next barrier at
  cultivation-year 1,000 — far, unfelt).
- Beat bank untouched: no leaps; the standings not staged; the wall unfelt.

## Status Delta — Chapter 14 close (DL 3664–65, the deep country's second winter; real age ≈ 68–70 months)
- Situation: THE WRONG FACE — the season of the watch; his own lying kills taught a
  rival reader; the men have put a face on the keepers' line, and the face is wrong.
- NATURE-MASTERY road: (a) THE SECOND EDGE OF THE SIGN — his lie-kills drew the
  wide-tracked pale beast of the deep fringe (meat untouched at a written kill);
  his own craft grown a second edge pointed at himself. (b) THE AIMED WORD — the
  voice's next truth: spent into a beast at four lengths instead of into the air
  (short, dense, no roll over the country); the pale beast stops and leaves the
  kill; the price — two days of the country reading too warmly, the thread loud
  in the chest, the walk home two days longer; a word said to a reader is a word
  the reader keeps. (c) THE WANT'S QUESTION — a heavy beast eaten deep and the
  hunger unmoved by the fullness; no meal can answer it again; it wants the
  standing itself; held like a current; DS-08 uncured.
- MEN ledger: THE WRONG FACE ON THE PAPER — the broken watch-fold (one man down
  at the water; the older lodge man died of it at the camp of black stone); Xiao
  An's argument that the printed beast is not the thing that signs; Old Qin's
  ruling (the count carries what can be proved; the mark and the beast written
  as one line until another proves otherwise); the line turned SOUTH on the wide
  pads — the watch pulled off his ground for the season; their paper now carries
  a killer with a heavy pale-sided shape and a name that waits.
- THE HOUND: the second reading — the crossing above the camp; two lines in the
  nose (the wide pads fresh; his own old scent from the water) and no way to make
  them one; the hound called off; the veiling held; the confusion is protection
  he never planned and cannot count on twice.
- Firewall: the men still have nothing of him; they believe the mark has a face,
  and the face is the pale beast's. The dragon never hears, never sees.
- Cultivation (panel; no clock, s7): REAL AGE ≈ 68–70 months; CULTIVATION-AGE
  ESTIMATE ≈ 190–205 (the winter's heavy meals; the aimed word's spend is not a
  gain). Next barrier at cultivation-year 1,000 — far, unfelt. No leaps.

## ch15 (DL 3665 — the thaw; the queued chapter delivered on the author's "Continue")
- THE THAW AND THE WAIT: water under the den; the camps on his ground down; one
  smoke line still south; he hunts the hard ground above and waits while it stands.
- THE LOW COUNTRY TAKEN: the smoke gone; the emptied watch read in one walk — the
  men killed a beast and left it standing, the pale beast came up out of the south
  and ate it, the men watched and went down the country; the wire and blind gone,
  the fire ring cold, the pale beast's tracks south and none returning; the worn
  circle of a hound that would not leave one patch of ground (he cannot read what
  she found); the ground open (no camp, no rival heavy beast, the meat coming down
  with the thaw, the road between the two countries); his own quiet over it; first
  kill on the new ground, the rest dragged off the trail.
- MEN ledger: THE SOUTH WATCH — four days on the low trail below the shelf of old
  rock, on the pale beast's chosen ground; the bait killed and left standing; the
  wire, the blind, the hound staked. THE BEAST COME TO THE MEAT: it stepped the
  wire as if it were drawn for it, stopped short of the kill, read the blind, ate
  in the open in daylight with the line counting it, and walked off south
  unhurried. THE RULING: the list carries what the ground can prove — the seen and
  counted beast written with the place under it, the old lodge man's name and pay
  under that; NO light for the table; the summer promise (Xiao An walks the high
  ground where the hound points, in front of Old Qin).
- THE HOUND: the third reading — the first at the summer water (ch12), the second
  on the high ground in the cold (ch14); the third time it leaves a living beast
  to stand over ground carrying his old scent and will not come off it. The one
  true reading in the south is the one the list cannot write.
- Firewall: still nothing of him on any paper; the men believe the paper's beast
  and now hold a direction (the summer walk) and a hound that will not give the
  scent up. The dragon never hears, never sees.
- Cultivation (panel; no clock, s7): REAL AGE ≈ 71–73 months; CULTIVATION-AGE
  ESTIMATE ≈ 195–210 (the winter's heavy meals and the thaw's new ground). Next
  barrier at cultivation-year 1,000 — far, unfelt. No leaps.

## ch16 (DL 3665 — the summer; the author's second " Continue ")
- THE SUMMER ON THE TAKEN GROUND: the low country hunted as his own (the old thin
  grazer taken at the lower pool at dusk; kills kept off the trail, the rest
  dragged into the fern, the crows paid); the heat, the flies, the water falling
  to pools; the hunger the same after every meal and only carried.
- THE LINE COMES BACK NORTH: smoke on the south wind; he reads them three days
  out; he lies on the rim and lets them walk his own trail, because a beast that
  moves in front of men is a beast they follow; the hound's night half-circle at
  the camp edge met by his quiet at its last limit; water skipped, two nights
  without a kill.
- MEN ledger: THE SUMMER WALK ON THE HIGH GROUND — stone, dry grass, baked pans;
  three days of nothing; the hound's stop at the dried pan; Xiao An's read (the
  print stood while the mud set, no roll in it, the beast looking down at the
  south); Gao Tie's casts off both ends (stone and stream; she will not cross
  running water); the print is NOT the pale beast's (narrower, heavier in front,
  one print in a whole season); Old Qin's ruling — one print with no face and no
  kill is not a line, the PAN IS THE WATCH, four stones set, next spring before
  the mud sets; the younger man's first watch promised; Xiao An's last look from
  the stones.
- THE TURN — HE PRINTS NO MORE: he reads the stopped places, the hound's two
  lines, the men's knees in the dust beside his own print; he does not touch the
  pan (a torn pan is one more thing to read), gives them nothing, and takes the
  long way — crossings at the rock bars, ridges of bare stone, mud flats left to
  the grazing kind; twice a kill passed up rather than cross soft mud, a day
  hungry each time; the cost paid.
- Firewall: the line now has a place and a shape and no beast; the pan will take
  a print from anything that walks in the wet, and next spring they will watch
  it. Nothing of him is written; but the ground itself now writes what he does
  not. The dragon never hears, never sees.
- Cultivation (panel; no clock, s7): REAL AGE ≈ 74–76 months; CULTIVATION-AGE
  ESTIMATE ≈ 200–215 (the summer's meat on the low water). Next barrier at
  cultivation-year 1,000 — far, unfelt. No leaps.

## ch17 (DL 3666 — the spring wet; the author's third " Continue ")
- THE WET AND THE TAKEN GROUND FLOODED: the streams over their banks; the low
  country one brown sheet; the flood's first gift (the slow kind stranded on
  the ridges, taken twice without a fight, chest-deep); then the meat gone and
  him hungry inside a week; the ravine den taken as his wet ground (the old
  roots den under the flood).
- THE RAIN'S HOURS (the wet's discipline): the wet country writes down
  everything, so he crosses open ground only in the heaviest rain and watches
  his own print slump and fill inside the hour; the long way by the rock bars;
  TWO KILLS PASSED UP and one mud-island beast left standing rather than sign
  soft ground; thin again inside the wet; the hunger using the thin weeks.
- MEN ledger: THE WATCH BY THE PAN — the three of them up before the mud sets;
  the wire low across both ways onto the pan; the lean-to; days all the same;
  the hound's rounds and the rain erasing her tracks; her nose to the place
  the one print had been and nothing left; the younger man's "we watch a bowl
  of soup"; Xiao An's "if". THE WIRE'S CATCH — the young ox of the low country
  in the wire at the black hour; the iron put down in the dark; half a night
  of poles; the kill clean and off the pan; the white light risen and boxed;
  the meat butchered on the rock. OLD QIN'S READING AND RULING — the pan's
  whole page; no heavy track but the ox's; no line gained for a season of
  rain; THE LOW WATER WATCH SET (when the flats come out, the line watches the
  water — the net now lies on his summer ground); the watch struck, the ground
  put back to rock, the four stones stay; "Do not put a line on the paper out
  of wanting one."
- HE READS THE STRUCK WATCH: their season baked into the pan and not one print
  of his; the price of printing nothing paid for a whole wet; the bare-rock
  border rule (he sleeps well back from mud).
- Firewall: the line has gained a watch method (the pan), a boxed white light,
  and a new ground to watch (the low water); it still has nothing of him — the
  only heavy track in the pan was the ox they brought themselves.
- Cultivation (panel; no clock, s7): REAL AGE ≈ 83–85 months; CULTIVATION-AGE
  ESTIMATE ≈ 205–215 (the flood's first gift and the kill in the drawn water;
  the lean weeks moved it little). Next barrier at cultivation-year 1,000 —
  far, unfelt. No leaps. (Open flag: hatch-year vs month-count reconciliation
  — foundation/OPEN.md item 19.)
