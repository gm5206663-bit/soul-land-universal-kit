...
Chapter 101 "What the Sword Was" = canon 286: the lake lesson (SWORD CONSCIOUSNESS crossed — tier 2 of the four realms; "we confer" = peer recognition; the old "weapon intent as a realm" label retracted 09-03ed vs a live master), the promotion ("we confer"), the love ledger spoken once, the two-word armor dream set, the Spirit Pagoda history fed to thread D22.
...
- **NEXT: canon 290 "Ceramic Mountain"** — novelfull slug `chapter-290-*.html` (novelfull numbering = canon+0; slug `ceramic-mountain`); novelhall ID **10716135** (novelhall numbering = canon same, IDs sequential from 266=10716111). Title-verify every fetch.
...
- Disk range: canon 229–289 held (`canon_extract/chapters/`). Fetch 290 BEFORE writing ch105 — canon is ore, fetched fresh, never recalled from model memory.
